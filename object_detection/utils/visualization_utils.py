...
from helpers.Positions import Ball
...
def visualize_boxes_and_labels_on_image_array(
    ...
    max_boxes_to_draw=20,
    min_score_thresh_ball=.5,
    min_score_thresh_pocket=.5,
    agnostic_mode=False,
    line_thickness=4,
    mask_alpha=.4,
  ...
  """
  ball_types = ["ball", "white", "yellow", "green", "brown", "blue", "pink", "black"]
  ...
  for i in range(boxes.shape[0]):
    ...
    if scores is None or (class_name in ball_types and scores[i] > min_score_thresh_ball) or (class_name == "pocket" and scores[i] > min_score_thresh_pocket):
  ...
  for box, color in box_to_color_map.items():
    ...
    width = image.shape[1]
    actual_xmin, actual_xmax, actual_ymin, actual_ymax = xmin * width, xmax * width, ymin * height, ymax * height
    if instance_masks is not None:
    ...
    # string is given like: "ball: 75%" so without_percentage removes the percentage string
    without_percentage = (box_to_display_str_map[box][0][0:-5])
    if without_percentage == "pocket":
        positions.pockets.append(((actual_xmin + actual_xmax)/2, (actual_ymin + actual_ymax)/2))

    else:
        positions.balls.append(Ball(without_percentage, ((actual_xmin + actual_xmax)/2, (actual_ymin + actual_ymax)/2)))
    if instance_boundaries is not None:
      draw_mask_on_image_array(
          image,
